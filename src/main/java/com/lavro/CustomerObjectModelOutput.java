...
import com.retail.CustomerObject.*;
...
public class CustomerObjectModelOutput {
    public static void main(String[] args) {
        ...
        final DatumWriter<CustomerObjectModel> datumWriter = new SpecificDatumWriter<>(CustomerObjectModel.class);
        ...
        try (DataFileWriter<CustomerObjectModel> dataFileWriter = new DataFileWriter<>(datumWriter)) {
            ...
            for(int i=0; i<1; i++){
                ...
                CustomerSource customerSource = CustomerSource.values()[new Random().nextInt(CustomerSource.values().length)];
                CustomerIdentity customerIdentity =CustomerIdentity.newBuilder()
                        ...
                        .setCreatedDate(faker.date().between(new Date(2021,0,1), new Date(2021,11,31)).toInstant())
                        .setUpdatedDate(faker.date().between(new Date(2022,0,1), new Date(2022,11,31)).toInstant())
                        .build();
                ...
                CustomerContentPreference customerContentPreference = CustomerContentPreference.newBuilder()
                        .setCustomerId(String.valueOf(customerId))
                        .setEmailContentPreference(ContentPreference.values()[new Random().nextInt(ContentPreference.values().length)])
                        .setEmailFrequency(MarketingFrequency.values()[new Random().nextInt(MarketingFrequency.values().length)])
                        .setPhoneContentPreference(ContentPreference.values()[new Random().nextInt(ContentPreference.values().length)])
                        .setPhoneFrequency(MarketingFrequency.values()[new Random().nextInt(MarketingFrequency.values().length)])
                        .setCreatedDate(faker.date().between(new Date(2021,0,1), new Date(2021,11,31)).toInstant())
                        .setUpdatedDate(faker.date().between(new Date(2022,0,1), new Date(2022,11,31)).toInstant())
                        .build();
                ...
                CustomerMemberShip customerMemberShip = CustomerMemberShip.newBuilder()
                        ...
                        .setStartDate(faker.date().between(new Date(2021,0,1), new Date(2021,11,31)).toInstant())
                        .setEndDate(faker.date().between(new Date(2022,0,1), new Date(2025,11,31)).toInstant())
                        .setCreatedDate(faker.date().between(new Date(2021,0,1), new Date(2021,11,31)).toInstant())
                        .setUpdatedDate(faker.date().between(new Date(2022,0,1), new Date(2022,11,31)).toInstant())
                        .build();
